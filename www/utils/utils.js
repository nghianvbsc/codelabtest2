...
exports.checkParamsValidate = function (params) {
    ...
    params.forEach(function (item, index) {
        ...
        } else if (item == null || item.trim().length == 0) {
...
exports.currencyFormat = function (number, fixLength) {
    ...
    if (fixLength == null) {
        ...
        r = (Number(number)).toLocaleString('en-US', {minimumFractionDigits: fixNum});
    } else {
        r = (Number(number)).toLocaleString('en-US', {minimumFractionDigits: fixLength});
    }
    r = r + '';
    r = r.split('.').join("x");
    r = r.split(',').join(".");
...
exports.currencyFormatPrice = function (number, fixLength) {
    let r = '';
    if (fixLength == null) {
        let stringNum = number + '';
        let arrInc = stringNum.split('.');
