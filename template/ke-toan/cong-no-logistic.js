var billShows = borrows
var currentPage = 1;
var itemsPerPage = 10;
var totalPages = Math.ceil(billShows.length / itemsPerPage)
var statusTable = 'all'
billShows.forEach(borrow=>{
    if (borrow) {
        switch (borrow.isPay) {
            case true:
                borrow.payText = 'Đã trả'
                break;
            case false:
                borrow.payText = 'Chưa trả'
                break;
        }
    }
})
function renderTable() {
    ...
    for (let i = start; i <= end; i++) {
        let borrow = billShows[i]
        if (borrow) {
            
            html += `
                <tr ${borrow._id}>
                    <td>${i + 1}</td>
                    <td><a style="color: #0058b7; background: unset" href="/chi-tiet-yeu-cau-vay.html/${borrow.billId}?logisticId=${borrow.logisticId}">${createMaDonHang(borrow.billName)}</a></td>
                    <td>${borrow.logisticName}</td>
                    <td>${borrow.logisticBorrowName}</td>
                    <td>${borrow.payText}</td>
                </tr>
            `
        }
    }
...
function filterList() {
    ...
    if (statusTable == 'all') {
        ...
        borrows.forEach(borrow => {
            if (statusTable == 1 && borrow.isPay) {
                billShows.push(borrow)
            } else if (statusTable == 0 && !borrow.isPay) {
                billShows.push(borrow)
            }
        })
    }
    ...
    billShows.forEach(item => {
        if (removeUtf8(item.logisticName).includes(key) || removeUtf8(item.logisticBorrowName.toString()).includes(key) || removeUtf8(item.payText.toString()).includes(key)) {
            dataBills.push(item)
        }
    })
    billShows = dataBills
