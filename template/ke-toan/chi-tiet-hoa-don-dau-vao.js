var billExports =[];
var billExportsShows = []
$(() => {
    let currentPage = 1;
    let itemsPerPage = 10;
    let totalPages = Math.ceil(billExports.length/itemsPerPage)
    ...
    function renderTable(page) {
        ...
        if (end > billExportsShows.length - 1) {
            end = billExportsShows.length
        }
        for (let i=start; i <= end; i++){
            if (billExportsShows[i]){
                html+=``
            }
        }
        $('.table').html(html);
        $('.pagination-bt .sort-number').text(`Hiển thị từ ${start+1} đến ${end+1} trong tổng số ${billExportsShows.length} kết quả`)
        renderPagination()
    }
