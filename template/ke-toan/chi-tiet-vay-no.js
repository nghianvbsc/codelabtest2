...
$(() => {
    let currentPage = 1;
    let itemsPerPage = 10;
    let totalPages = Math.ceil(loans.length/itemsPerPage)
    ...
    function renderTable(page) {
        ...
        if (end > loansShows.length - 1) {
            end = loansShows.length
        }
        ...
        $('.table').html(html);
        $('.pagination-bt .sort-number').text(`Hiển thị từ ${start+1} đến ${end+1} trong tổng số ${loansShows.length} kết quả`)
        renderPagination()
    }
