...
function renderTable() {
    ...
    for (let i = start; i <= end; i++) {
        ...
        if (bill) {
            if ([0, 1, 6, 3, 4].includes(userLogin.type)) {
                html += `
                <tr>
                    <td>${i + 1}</td>
                    ...
                    <td><a style="color: #0058b7; background: unset" href="/nguoi-dung-quan-ly-don-hang.html?userId=${bill.userId}">${bill.userName}</a></td>
                    <td>${bill.typeT}</td>
                    <td>${bill.statusText}</td>
                </tr>
            `
            } else if (userLogin.type == 7) {
                html += `
                <tr>
                    <td>${i + 1}</td>
                    <td><a style="color: #0058b7 background: unset" href="/chi-tiet-hop-dong.html?contractId=${bill.contractId}">${bill.contractName}</a></td>
                    <td><a style="color: #0058b7; background: unset" href="/chi-tiet-don-hang/${bill._id}.html">${createMaDonHang(bill.name)}</a></td>
                    <td>${moment(bill.createAt).format('HH:mm DD/MM/YYYY')}</td>
                    <td><a style="color: #0058b7; background: unset" href="/danh-sach-hang-user-dat.html/${bill.userId}">${bill.customerName}</a></td>
                    <td>${bill.typeT}</td>
                    <td>${bill.statusText}</td>
                </tr>
            `
...
function filterList() {
    ...
    let key = removeUtf8($('#search-goods').val().toString())
    let dataBills = []
