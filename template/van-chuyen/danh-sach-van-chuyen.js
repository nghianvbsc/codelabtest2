...
$(() => {
    let currentPage = 1;
    let itemsPerPage = 10;
    let totalPages = Math.ceil(transports.length / itemsPerPage);
    ...
    function renderTable(page) {
        ...
        $('.table').html(html);
        $('.pagination-bt .sort-number').text(`Hiển thị từ ${start + 1} đến ${end + 1} trong tổng số ${transportShows.length} kết quả`)
        renderPagination()
    }
