var productBorrows =[];
var productBorrowsShows = []
$(() => {
    let currentPage = 1;
    let itemsPerPage = 10;
    let totalPages = Math.ceil(productBorrows.length/itemsPerPage)
    ...
    function renderTable(page) {
        ...
        if (end > productBorrowsShows.length - 1) {
            end = productBorrowsShows.length
        }
        for (let i=start; i <= end; i++){
            if (productBorrowsShows[i]){
                html+=``
            }
        }
        $('.table').html(html);
        $('.pagination-bt .sort-number').text(`Hiển thị từ ${start+1} đến ${end+1} trong tổng số ${productBorrowsShows.length} kết quả`)
        renderPagination()
    }
