    ...
    for (let e in contract.listCategories) {
        contract.listCategories[e].products.forEach(product => {
            if (product.index == indexProduct) {
                ...
                let priceWithVAT = Number(Number(product.price) + Number(product.price) * Number(product.tax)).toFixed(0);
...
function taoDonHang() {
    ...
    post(`/tao-don-hang-moi-cho-khach-hang/${contract._id}.html`, {
        ...data,
        ...
        if (success.error) {
            ...
            post(`/tao-don-hang-moi-cho-khach-hang/${contract._id}.html`, {
                ...
                if (success.error) {
                    ...
                    setTimeout(() => {
                        location.href = `/quan-ly-don-hang.html?contractId=${contract._id}`;
                    }, 2000);
                }
            }, error => {
...
$(() => {
        ...
        if (userId != 'null') {
            ...
            $('.item-category').each(function () {
                customers.forEach(item => {
                    if (item._id == userId) {
                        $('.form-info-receiver input[name="fullName"]').val(item.fullName)
                        $('.form-info-receiver input[name="phone"]').val(item.phone)
                        $('.form-info-receiver input[name="address"]').val(item.address)
                    }
                })
            });
        }
    ...
    $('.cancel-order').on('click', function () {
        window.href = `/quan-ly-don-hang.html?contractId=${contract._id}`;
    })
    renderListProduct()
})