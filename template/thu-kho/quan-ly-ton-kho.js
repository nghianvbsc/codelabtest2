...
$(() => {
    ...
    function renderTable(page) {
        ...
        if (end > productsShows.length - 1) {
            end = productsShows.length
        }
        ...
        $('.table').html(html);
        $('.pagination-bt .sort-number').text(`Hiển thị từ ${start+1} đến ${end+1} trong tổng số ${productsShows.length} kết quả`)
        renderPagination()
    }
