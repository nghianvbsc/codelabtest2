...
$(() => {
    let currentPage = 1;
    let itemsPerPage = 10;
    let totalPages = Math.ceil(warehouses.length/itemsPerPage)
    ...
    function renderTable(page) {
        ...
        if (end > warehousesShows.length - 1) {
            end = warehousesShows.length
        }
        for (let i=start; i <= end; i++){
            if (warehousesShows[i]){
                html+=``
            }
        }
        $('.table').html(html);
        $('.pagination-bt .sort-number').text(`Hiển thị từ ${start+1} đến ${end+1} trong tổng số ${warehousesShows.length} kết quả`)
        renderPagination()
    }
