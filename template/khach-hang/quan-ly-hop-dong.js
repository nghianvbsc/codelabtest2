...
$(() => {
    let currentPage = 1;
    let itemsPerPage = 10;
    let totalPages = Math.ceil(contracts.length/itemsPerPage)
    ...
    function renderTable(page) {
        ...
        if (end > contractsShows.length - 1) {
            end = contractsShows.length
        }
        ...
        $('.table').html(html);
        $('.pagination-bt .sort-number').text(`Hiển thị từ ${start+1} đến ${end+1} trong tổng số ${contractsShows.length} kết quả`)
        renderPagination()
    }
