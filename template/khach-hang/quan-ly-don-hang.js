...
$(() => {
    let currentPage = 1;
    let itemsPerPage = 10;
    let totalPages = Math.ceil(bills.length/itemsPerPage)
    ...
    function renderTable(page) {
        ...
        if (end > billsShows.length - 1) {
            end = billsShows.length
        }
        ...
        $('.table').html(html);
        $('.pagination-bt .sort-number').text(`Hiển thị từ ${start+1} đến ${end+1} trong tổng số ${billsShows.length} kết quả`)
        renderPagination()
    }
